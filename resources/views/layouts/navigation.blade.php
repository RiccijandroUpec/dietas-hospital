    ...
    <div :class="[
        ...
        <nav class="mt-4 space-y-1 px-2">
            ...
            @if(auth()->check() && auth()->user()->role === 'admin')
                ...
                <!-- Admin Collapsible -->
                <div x-data="{ adminOpen: {{ request()->routeIs('usuarios.*', 'camas.*', 'servicios.*', 'tipos-dieta.*') ? 'true' : 'false' }} }">
                    ...
                    <div x-show="adminOpen" class="pl-8 mt-1 space-y-1" :class="!sidebarOpen && 'lg:hidden'">
                        ...
                        <a href="{{ route('camas.index') }}" class="flex items-center px-4 py-2 rounded-lg text-sm transition-all {{ request()->routeIs('camas.*') ? 'bg-blue-600 text-white' : 'text-blue-100 hover:bg-blue-700' }}">
                            <svg class="h-4 w-4 me-2 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
