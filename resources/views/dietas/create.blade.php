    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <h2 class="font-semibold text-xl text-gray-800 mb-4">Crear Dieta</h2>
                ...
                <form action="{{ route('dietas.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <select name="tipo_dieta_id" class="mt-1 block w-full border-gray-300 rounded-md" id="tipo_dieta_id">
                                ...
                                @foreach($tipos as $tipo)
                                    <option value="{{ $tipo->id }}" {{ old('tipo_dieta_id') == $tipo->id ? 'selected' : '' }}>{{ $tipo->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="subtipo_dieta_id[]" class="mt-1 block w-full border-gray-300 rounded-md" id="subtipo_dieta_id" multiple>
                                @foreach($subtipos as $subtipo)
                                    <option value="{{ $subtipo->id }}" {{ (is_array(old('subtipo_dieta_id')) && in_array($subtipo->id, old('subtipo_dieta_id'))) ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <textarea name="descripcion" class="mt-1 block w-full border-gray-300 rounded-md" rows="5">{{ old('descripcion') }}</textarea>
                            @error('descripcion')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
                        </div>

                        <div class="pt-4 flex gap-2">
                            <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md font-medium transition">💾 Guardar</button>
