    ...
    <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <h2 class="font-semibold text-2xl text-gray-800 mb-2">➕ Crear Subtipo de Dieta</h2>
                <p class="text-gray-600 text-sm mb-6">Agregar una nueva subcategoría de dieta</p>

                <form action="{{ route('subtipos-dieta.store') }}" method="POST">
                    @csrf
                        ...
                        <div>
                            ...
                            <select name="tipo_dieta_id" class="mt-1 block w-full border-gray-300 rounded-md" required>
                                ...
                                @foreach($tipos as $tipo)
                                    <option value="{{ $tipo->id }}" @selected(old('tipo_dieta_id') == $tipo->id)>{{ $tipo->nombre }}</option>
                                @endforeach
                            </select>
